...
export default class Level8 extends BaseScene
{
    ...
    {
        super.create({
            title: 'Уровень: 8. Умножение',
            time: 60,
            background: 'gorny',
            instruction: 'Реши пример и поймай всех стрекоз с правильным ответом!'
        });
        ...
        this.platform1 = makePlatform(710, 380, 'platform_mini1');
        ...
        this.platform6 = makePlatform(150, 160, 'platform_mini1');

        // =====================
        // ГЕНЕРАЦИЯ ПРИМЕРА (умножение двух чисел, максимум 125)
        // =====================
        this.a = Phaser.Math.Between(2, 5);
        ...
        {
            const offset = Phaser.Math.Between(-5, 5); // случайное смещение
        ...
        this.answerPositions.forEach((pos, index) =>
        {
            ...
            const item = this.answerGroup.create(pos[0], pos[1], 'dragonfly')
                .setScale(0.25);
            ...
            item.nextFlip = this.time.now + Phaser.Math.Between(100, 500);
    ...
    update()
    {
        ...
        // Анимация объектов (СМЕНА НАПРАВЛЕНИЯ)
        this.answerGroup.children.iterate(item =>
        {
            const baseScale = 0.25; // базовый размер

            //item.setScale(baseScale + Math.sin(this.time.now * 0.002) * 0.02); // пульсация
    ...
    onWin()
    {
        ...
        this.markCompleted('Level8'); // отмечаем, что уровень прошел
