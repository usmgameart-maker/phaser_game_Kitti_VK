...
export default class Level3 extends BaseScene
{
    ...
    create(data)
    {
        super.create({ // вызываем метод create() родительского класса
           title: 'Уровень: 3. Сложение', // название уровня
            time: 40, // время игры
            background: 'lawn',
            instruction: 'Реши пример и собери конфеты с правильным ответом, пока они не исчезли!' // инструкция для игрока. Это задание
        ...
        // =====================
        // ГЕНЕРАЦИЯ ПРИМЕРА (сложение двухзначных чисел, максимум 100)
        // =====================
        this.a = Phaser.Math.Between(25, 50);
        ...
        this.answerPositions.forEach((pos, index) =>
        {
            ...
            const item = this.answerGroup.create(pos[0], pos[1], 'candy')
                .setScale(0.25);
        ...
        this.answerGroup.children.each((candy, index) => // проходим по всем конфетам
        {
    ...
    onWin()
    {
        ...
        this.markCompleted('Level3'); // отмечаем, что уровень прошел
