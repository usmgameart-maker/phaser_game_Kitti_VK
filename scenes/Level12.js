...
export default class Level12 extends BaseScene
{
    constructor()
    {
        super('Level12');
    ...
    create(data)
    {
        super.create({ // вызываем метод create() родительского класса
           title: 'Уровень: 4. Деление', // название уровня
            time: 60, // время игры
            background: 'garden',
            instruction: 'Реши пример и собери шишки с правильным ответом, пока они не исчезли!' // инструкция для игрока. Это задание
        ...
        this.platform3 = makePlatform(730, 420, 'platform_mini1');
        ...
        // =====================
        // ГЕНЕРАЦИЯ ПРИМЕРА (сложение двухзначных чисел, максимум 200)
        // =====================
        this.b = Phaser.Math.Between(2, 9); // делитель
        this.correctAnswer = Phaser.Math.Between(10, 20); // ответ
        ...
        this.answerPositions.forEach((pos, index) =>
        {
            ...
            const item = this.answerGroup.create(pos[0], pos[1], 'cone')
                .setScale(0.25);
        ...
        this.answerGroup.children.each((candy, index) => // проходим по всем объектам
        {
    ...
    {
        if (this.gameEnded) return; // если игра уже закончилась, не проверяем
    ...
    onWin()
    {
        ...
        this.markCompleted('Level12'); // отмечаем, что уровень прошел
    ...
    onLose()
    {
        ...
        this.time.delayedCall(300, () => // перезапускаем эту же сцену
        {
