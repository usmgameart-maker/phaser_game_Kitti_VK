...
export default class Level11 extends BaseScene
{
    constructor()
    {
        super('Level11');
    ...
    {
        super.create({
            title: 'Уровень: 11. Деление',
            time: 60,
            background: 'garden',
            instruction: 'Реши пример и собери все листочки с правильным ответом!'
        });
        ...
        this.platform3 = makePlatform(70, 420, 'platform_mini1');

        this.platform4 = makePlatform(620, 200, 'platform_mini2');

        this.platform5 = makePlatform(250, 280, 'platform_mini2');

        this.platform6 = makePlatform(150, 150, 'platform_mini1');

        // =====================
        // ГЕНЕРАЦИЯ ПРИМЕРА (деление..........)
        // =====================
        this.b = Phaser.Math.Between(5, 10); // делитель
        this.correctAnswer = Phaser.Math.Between(5, 10); // ответ

        this.a = this.b * this.correctAnswer; // делимое
        ...
        this.exampleText = this.add.text(
            400,
            80,
            `${this.a} : ${this.b} = ❓`,
            {
        ...
        {
            this.answers.push(w); // добавляем неправильные ответы
            this.answers.push(w); // добавляем неправильные ответы
        });
        ...
        // =====================
        // условие: ПРАВИЛЬНО СОБРАНО / ВСЕ СОБРАНО
        // =====================

        this.collectedCorrect = 0; // количество собранных правильных ответов
        this.totalCorrect = 2; // общее количество правильных ответов
        ...
        this.answerPositions.forEach((pos, index) =>
        {
            ...
            const item = this.answerGroup.create(pos[0], pos[1], 'leaf')
                .setScale(0.25);
        ...
        // OVERLAP. КОЛЛИЗИЯ ИГРОКА С ПРЕДМЕТАМИ (проверяем столкновение игрока с ответами)
    ...
    update()
    {
        ...
        // Анимация объектов (вращение+пульсация)
        this.answerGroup.children.iterate(item =>
        {
            item.angle += 2; // скорость вращения

            const baseScale = 0.25; // базовый размер
            item.setScale(baseScale + Math.sin(this.time.now * 0.005) * 0.03); // пульсация
        });
    }
    ...
    onWin()
    {
        ...
        this.markCompleted('Level11'); // отмечаем, что уровень прошел
