...
export default class Level10 extends BaseScene
{
    ...
    create(data)
    {
        super.create({ // вызываем метод create() родительского класса
            ...
            title: 'Уровень: 10. Деление', // название уровня
            time: 40, // время игры
            background: 'garden', // фон уровня
            music: 'bg_music', // музыка уровня
            instruction: 'Реши пример и собери все цветы с правильным ответом!' // инструкция для игрока. Это задание
        ...
        this.platform2 = makePlatform(140, 490, 'chest'); // сундук, подставка
        this.platform3 = makePlatform(460, 420, 'platform_mini2');

        this.platform4 = makePlatform(780, 480, 'platform_mini1');
        ...
        // =====================
        // ГЕНЕРАЦИЯ ПРИМЕРА (деление двух чисел, по таблице умножения)
        // =====================
        this.b = Phaser.Math.Between(2, 5); // делитель
        this.correctAnswer = Phaser.Math.Between(2, 5); // ответ
        ...
        this.answerPositions.forEach((pos, index) =>
        {
            ...
            const item = this.answerGroup.create(pos[0], pos[1], 'flower_2');
            item.setScale(0.25);
    ...
    onWin()
    {
        ...
        this.markCompleted('Level10'); // отмечаем, что уровень прошел
